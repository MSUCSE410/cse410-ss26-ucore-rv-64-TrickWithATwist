...
/*
*       LAB5: (3) deadlock detection using banker's algorithm
*/
                    ...
                    const int request[NTHREAD][LOCK_POOL_SIZE])
{
        ...
        // If any thread can't finish, deadlock detected
        for (int i = 0; i < NTHREAD; i++) {
...
int sys_mutex_lock(int mutex_id)
{
        ...
        // After acquiring: update allocation, clear request, decrement available
        if (p->deadlock_detect_enabled) {
...
int sys_mutex_unlock(int mutex_id)
{
        ...
        struct proc *p = curr_proc();
        int tid = curr_thread()->tid;
        // LAB5: (4-1) update detection variables
        if (p->deadlock_detect_enabled) {
                p->mutex_available[mutex_id] = 1;
...
int sys_semaphore_create(int res_count)
{
        ...
        // LAB5: (4-2) initialize detection variables
        struct proc *p = curr_proc();
        p->sem_available[sem_id] = res_count;
        for (int i = 0; i < NTHREAD; i++) {
                p->sem_allocation[i][sem_id] = 0;
                p->sem_request[i][sem_id] = 0;
        }
...
int sys_semaphore_up(int semaphore_id)
{
        ...
        struct proc *p = curr_proc();
        int tid = curr_thread()->tid;
        // LAB5: (4-2) update detection variables
        if (p->deadlock_detect_enabled) {
